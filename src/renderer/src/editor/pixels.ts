...
export function adjustLayer(doc: Doc, id: string, a: Adjustments | null, f: Filters | null): Doc {
  return editPixels(doc, id, 'layer', undefined, (d, w, h) => {
    if (a && hasAdjust(a)) applyAdjustments(d, a, 7)
    if (f && hasFilters(f)) d.set(applyFilters(d, w, h, f, 7, 'clamp'))
  })
}
