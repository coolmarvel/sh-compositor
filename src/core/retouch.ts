...
import { tipAlpha, type BrushSettings } from './doc/brush'
...
export function blurDab(s: RetouchSession, cx: number, cy: number): { x: number; y: number; w: number; h: number } | null {
  ...
  const patch = snapshot(s.live, x0 - k, y0 - k, x1 + k, y1 + k)
    ...
    for (let x = x0; x <= x1; x++) {
      ...
      for (let j = -k; j <= k; j += Math.max(1, k >> 1))
        for (let i = -k; i <= k; i += Math.max(1, k >> 1)) {
          ...
          const o = ((yy - patch.y) * patch.width + xx - patch.x) * 4
          const w = src[o + 3]
          r += src[o] * w
          g += src[o + 1] * w
          b += src[o + 2] * w
...
export function smudgeDab(s: RetouchSession, from: Pt, to: Pt): { x: number; y: number; w: number; h: number } | null {
  ...
  const patch = snapshot(s.live, x0 - Math.max(0, dx) - 1, y0 - Math.max(0, dy) - 1, x1 - Math.min(0, dx) + 1, y1 - Math.min(0, dy) + 1)
  const src = patch.data
  for (let y = y0; y <= y1; y++)
    for (let x = x0; x <= x1; x++) {
      ...
      const so = ((sy - patch.y) * patch.width + sx - patch.x) * 4
      const o = (y * W + x) * 4
      ...
      for (let c = 0; c < 4; c++) {
        ...
        for (let j = 0; j < 2; j++)
          for (let i = 0; i < 2; i++) {
            ...
            v += src[((yy - patch.y) * patch.width + xx - patch.x) * 4 + c] * (i ? fx : 1 - fx) * (j ? fy : 1 - fy)
          }
        d[o + c] = v
      }
    }
